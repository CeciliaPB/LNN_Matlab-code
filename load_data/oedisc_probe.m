function [AllTimeStamps, AllWaveForms] = oedisc_probe(data,ts,sr,thr,varargin)
...
switch A
    ...
    case 0
        NumTetrodes = 1;   % number of tetrodes - 4
        [tvdisc, tpeaks, AllTimeStamps, AllWaveForms] = deal(cell(1,NumTetrodes));
        for iT = 1:NumTetrodes
        [cvdisc, tdata, cpeaks] = deal(cell(1,NumChannels));
        ...
        % Waveform
        winx = repmat(tvdisc{iT}(:)+winp(1),1,sum(abs(winp))) + repmat(0:sum(abs(winp))-1,length(tvdisc{iT}),1);
        wv = nan(size(winx,1),NumChannels,size(winx,2));   % waveforms: spikes x channels x time
        for iC = 1:NumChannels
            wv(:,iC,:) = tdata{iC}(winx);   % waveform data
        end
        WaveForms = wv;
