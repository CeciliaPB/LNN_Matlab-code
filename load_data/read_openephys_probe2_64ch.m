...
switch g.reference
    case 'common_avg'
        common_avg = common_avg_ref_probe2(g.datadir,g.CHspec,[],g.rawdatafiletag,'processor', g.processor);
    case {'','none'}
        common_avg = 0;
    otherwise
...
for iT = g.TTspec
    ...
    for iC = 1:4
        Ch1 = (iT - 1) * 4;
        [tdata{iC}, ts{iC}, info] = load_open_ephys_data([ g.datadir '\' num2str(g.processor) '_' num2str(Ch1+iC) g.rawdatafiletag '.continuous']);
    end
...
for iT = 1:max(g.TTspec)-1
    ...
    % Channel 1
    Ch1 = (iT * 4) ;
    [tdata1, ts1, ~] = load_open_ephys_data([ g.datadir '\' num2str(g.processor) '_' num2str(Ch1) g.rawdatafiletag '.continuous']);
    
    % Channel 2
    Ch2 = Ch1 + 1 ;
    [tdata2, ts2, info] = load_open_ephys_data([ g.datadir '\' num2str(g.processor) '_' num2str(Ch2) g.rawdatafiletag '.continuous']);
