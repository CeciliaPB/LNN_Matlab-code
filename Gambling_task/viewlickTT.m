...
switch (g.LickSide)
    case 'all' 
        ...
        stimes = arrayfun(@(s)LickIn{s}-TE.(g.TriggerName)(s),1:NumTrials,'UniformOutput',false);
    case 'R'
        NumTrials = length(TE.LickRIn);
        stimes = arrayfun(@(s)TE.LickRIn{s}-TE.(g.TriggerName)(s),1:NumTrials,'UniformOutput',false);
    case 'L'
        NumTrials = length(TE.LickLIn);
        stimes = arrayfun(@(s)TE.LickLIn{s}-TE.(g.TriggerName)(s),1:NumTrials,'UniformOutput',false);
end
...
[mylabels, mycolors, mycolors2,mylinestyle] = makeColorsLabels(@defineLabelsColors_Cecilia,newTAGS);
