function tagging_YBlasers(I,issave)
...
% Directories
resdir = 'L:\Cecilia\Behaviour\Risk assesment';
xlsname = [resdir filesep 'tagging_data.xls'];   % write results to excel file
...
for k = 1:NumCells
    ...
    try
        ...
        if logical(exist(cellid2fnames(cellid,'STIMSPIKES'),'file'))
            isstim = 1;
        else
        ...
        if ~skipclusterquality
            ...
            [ID_PC, Lr_PC, valid_channels] = LRatio(cellid,feature_names2); 
        else   % if cluster quality was already calculated, it can be skipped
            [ID_amp, Lr_amp] = deal(NaN);
            [ID_PC, Lr_PC, valid_channels] = deal(NaN); 
        end
        ...
        if isstim
            ...
            [bHindex, bD_KL] = nbisstim_YBlasers(cellid,'blue');
            [yHindex, yD_KL] = nbisstim_YBlasers(cellid,'yellow');
        ...
        if issave
            ...
            save([resdir 'TAGGING_' regexprep(cellid,'\.','_') '.mat'],...
                ...
                'Hindex','D_KL','bHindex','bD_KL','yHindex','yD_KL','R')
            ...
            bHindex_xls = formatforExcel(bHindex);
            yHindex_xls = formatforExcel(yHindex);
            D_KL_xls    = formatforExcel(D_KL);
            bD_KL_xls   = formatforExcel(bD_KL);
            yD_KL_xls   = formatforExcel(yD_KL);
            ...
            xlswrite(xlsname,bHindex_xls,'sheet1',['H' num2str(k)])
            xlswrite(xlsname,bD_KL_xls,  'sheet1',['I' num2str(k)])
            xlswrite(xlsname,yHindex_xls,'sheet1',['J' num2str(k)])
            xlswrite(xlsname,yD_KL_xls,  'sheet1',['K' num2str(k)])
            xlswrite(xlsname,R_xls,      'sheet1',['L' num2str(k)])
        end
