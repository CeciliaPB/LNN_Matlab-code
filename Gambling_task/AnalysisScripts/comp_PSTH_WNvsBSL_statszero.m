...
%         legend(mylabels,'Box','off','Location','northwest');
    ...
    if exist('clri','var')
        ...
        if testwin(1) < 0
            ...
            hI(iP) = plot(newt(crossi_dw_inx:crossi_up_inx),psth_t(crossi_dw_inx:crossi_up_inx),'color',...
                mycolors{iP},'LineWidth',2);
        else
            hI(iP) = plot(time_t(crossi_dw_inx:crossi_up_inx),psth_t(crossi_dw_inx:crossi_up_inx),'color',...
                mycolors{iP},'LineWidth',2);
        end
        x_lim = xlim;
        y_lim = ylim;
        text(x_lim(1)+(x_lim(2)-x_lim(1))*0.4,y_lim(1)+(y_lim(2)-y_lim(1))*ytx_multiplier1{iP},...
    ...
    if exist('clra','var')
        ...
        if testwin(1) < 0
            newt   = abs(flip(time_t));
            hA(iP) = plot(newt(crossa_up_inx:crossa_dw_inx),psth_t(crossa_up_inx:crossa_dw_inx),'color',...
                mycolors{iP},'LineWidth',2);
        else
            hA(iP) = plot(time_t(crossa_up_inx:crossa_dw_inx),psth_t(crossa_up_inx:crossa_dw_inx),'color',...
                mycolors{iP},'LineWidth',2);
        end
        x_lim = xlim;
        y_lim = ylim;
        text(x_lim(1)+(x_lim(2)-x_lim(1))*0.4,y_lim(1)+(y_lim(2)-y_lim(1))*ytx_multiplier2{iP},...
