function [p_value, Idiff] = nbisstim_YBlasers(cellid,laser)
...
if nargin == 2
    ...
    dt    = 0.001;   % resolution of bin raster in s (for Hyun: 0.0005)
    dsply = 0;   % repress display
    if strcmp(laser,'blue') == 1
        laserID = 1;
    elseif strcmp(laser,'yellow') == 1
        laserID = 2;
    end
end
...
if strcmp(laserID,'all') == 1
    stimes1 = ST.event_stimes{epoch_pos1};
    stimes2 = ST.event_stimes{epoch_pos2};
elseif laserID == 1 || laserID == 2
    laserON = double(TE.PulseID == laserID);
    stimes1 = ST.event_stimes{epoch_pos1};
    stimes2 = ST.event_stimes{epoch_pos2};
    for ii = 1:length(laserON)
        if laserON(ii) == 0
            ...
            stimes2{ii} = {};
        elseif laserON(ii) == 1
            stimes1(ii) = stimes1(ii);
            stimes2(ii) = stimes2(ii);
        end
    end
end
