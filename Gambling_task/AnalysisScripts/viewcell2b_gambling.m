function viewcell2b_gambling(cellid,varargin)
...
%
%   Based on VIEWCELL2B by Balazs Hangya   Edit log: BH 6/23/11, 2/10/12
% -------------------------------------------------------------------------
...
%     binraster2 = nan(NUMtrials,size(binraster0,2));
    for k = 1:NUMtrials   % calculate sum of rows for each trial, used for the PSTH
...
elseif ~isempty(g.SortEvent)
    if ~iscell(TE.(g.TriggerEvent))
        ...
        gte(inx) = cell2mat(cellfun(@(s)s(1),TE.(g.TriggerEvent)(inx),...
            'UniformOutput',false));
