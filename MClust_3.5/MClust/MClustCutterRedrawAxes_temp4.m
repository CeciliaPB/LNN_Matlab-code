...
% Prevent retriggering by double-click
prec = 0.5;
prev_zoom = getappdata(gca,'last_zoom');
if isempty(prev_zoom)
    prev_zoom = clock - 1;
end
this_zoom = clock;
setappdata(gca,'last_zoom',this_zoom);
tn = clock;
if etime(tn,prev_zoom) < prec     % this is because a double-click retriggers the callback
    return
end
