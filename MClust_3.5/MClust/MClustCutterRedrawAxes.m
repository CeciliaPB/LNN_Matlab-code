...
global MClust_PlotSkipPoints
MClust_PlotSkipPoints = min(30,size(MClust_CurrentFeatureData,1)+1);   % disregard the largest 30 values on both axes
...
x0 = MClust_CurrentFeatureData(:,1);
sx = sort(x0);
mnx = sx(1);
mxx = sx(end-MClust_PlotSkipPoints+1);   % disregard the largest 30 values on x axis
y0 = MClust_CurrentFeatureData(:,2);
sy = sort(y0);
mny = sy(1);
mxy = sy(end-MClust_PlotSkipPoints+1);   % disregard the largest 30 values on y axis
...
if full %%% Added by JCJ Aug 2007 to stabilize redraw of display
    set(A, 'XLim', [mnx mxx+0.0001]);
    set(A, 'YLim', [mny mxy+0.0001]);	
else
    axis(curAxis);
end
for iC = 0:nClust
...
% Overlay light spikes
global ShowLightSpikes
if isempty(ShowLightSpikes)
    ShowLightSpikes = false;
end
if ShowLightSpikes
    overlayspk
end
...
if isempty(LightSpikes)
    
    % Directory
    fn = MClust_TTdn;
    
    % Events
    switch MClust_NeuralLoadingFunction
        case 'LoadTT_NeuralynxNT'
        ...
        case 'LoadTT_Intan'
            fnm = fullfile(fn,'lightevent');
            load(fnm)
            light_ts = pulseon * 10^4;
            
        otherwise
            error('Light evoked spikes: loading engine not recognized.')
    end
